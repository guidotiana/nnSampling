...
from samplers.rphmc_sampler import RPHMCSampler
...
def get_exists(extra, pars):
    ...
    check_wfiles = [f'r{i}_{extra["prefix"]}0.pt' in os.listdir(extra['weights_dir']) for i in range(pars['y'])]
    return all(check_files + check_wfiles)


# Restart previous simulation
def restart(extra, pars):
    ...
    last['files'] = {
            'models': {
                f'r{i}': f'{extra["weights_dir"]}/r{i}_{extra["prefix"]}{last_wmove}.pt' for i in range(pars['y'])
            },
...
def summary(spars, extra, models, title):
    trainable = ''
    for name in models['r0'].weights: trainable = f'{trainable}, {name}'
    ...
    lines.append(f'# replica gamma:     {format(spars["gamma"], ".1e")}')
...
def main(args):
    ...
    print('Initializing replica models...')
    models = {}
    for i in range(pars['y']):
        models[f'r{i}'] = CMModel(
                N_k=pars['N_k'],
                K=pars['K'],
                activ=pars['activ'],
                seed=pars['model_seed']*(i+1),
                device=extra['device'],
                f=pars[f'r{i}_from'],
        )
    ...
    tot_moves = 0
    for i, (stime, T, gamma, dt, isteps, m) in enumerate(zip(pars['stime_list'], pars['T_list'], pars['gamma_list'], pars['dt_list'], pars['isteps_list'], pars['m_list'])):
