...
def prepare_directory(args):
	...
	pars['model']['start_fns'] = {
		f'{end}': pars['model'][f'{end}_from'] for end in ['fe', 'se']
	}
	...
	models = {
		f'{end}': NNModel(
			ComMachine(
				...
				seed=pars['model']['model_seed']+iend, #to avoid having a dictionary composed of the same models
			)
		) for iend,end in enumerate(['fe', 'se'])
	}
